...
class Runner():
    def __init__(self, args):
        ...
        self.prior_set, self.prior_sampler, self.prior_loader = create_data(
            self.args.prior, self.args.gpus,
            dataset_size=base_steps_per_epoch * self.args.batch_size,
            batch_size=self.args.batch_size
        )
        self.data_set, self.data_sampler, self.data_loader = create_data(
            self.args.dataset, self.args.gpus,
            dataset_size=base_steps_per_epoch*self.args.batch_size,
            batch_size=self.args.batch_size
        )
        self.prior_iterator, self.data_iterator = \
            iter(self.prior_loader), iter(self.data_loader)
        ...
        if not self.args.exp2d:
            ...
            if self.args.val_prior is not None and self.args.val_dataset is not None:
                self.val_prior_set, _, _ = create_data(
                    self.args.val_prior, self.args.gpus,
                    dataset_size=base_steps_per_epoch * self.args.batch_size,
                    batch_size=self.args.batch_size
                )
                self.val_data_set, _, _ = create_data(
                    self.args.val_dataset, self.args.gpus,
                    dataset_size=base_steps_per_epoch * self.args.batch_size,
                    batch_size=self.args.batch_size
                )
        ...
        if self.dsb:
            ...
            self.backward_model = create_model(
                self.args.method, self.args,
                self.device, self.noiser,
                rank=self.rank, direction='b'
            )
            self.forward_model = create_model(
                self.args.method, self.args,
                self.device, self.noiser,
                rank=self.rank, direction='f'
            )
            if self.rank == 0:
                print(
                    f'Backward Network #Params: {sum([p.numel() for p in self.backward_model.parameters()])}'
                )
                print(
                    f'Forward Network #Params: {sum([p.numel() for p in self.forward_model.parameters()])}'
                )
            ...
            self.model = {'backward': self.backward_model,
                          'forward': self.forward_model}
            self.optimizer = {'backward': self.backward_optimizer,
        ...
        if self.args.global_rank == 0:
            ...
            self.logger = Logger(
                ...
                self.noiser.training_timesteps
            )
    ...
    def next_batch(self, epoch, dsb=False):
        if dsb:
            if self.cnt + self.args.batch_size > self.cache_size:
                ...
                num_cache = math.ceil(
                    self.cache_size / self.args.batch_size / self.noiser.num_timesteps)
                pbar = tqdm.trange(
                    num_cache, desc=f'Cache epoch {epoch} model {self.direction}') if self.rank == 0 else range(num_cache)
                for _ in pbar:
                    x_0, x_1 = self._next_batch()
                    with torch.no_grad():
                        if self.direction == 'backward' and epoch == 0:
                            _x_cache, _gt_cache, _t_cache = self.noiser.trajectory_dsb(
                                x_0, x_1)
                        else:
                            model = self.model['backward' if self.direction ==
                                               'forward' else 'forward']
                            model.eval()
                            _x_cache, _gt_cache, _t_cache = model.inference(
                                x_0 if self.direction == 'forward' else x_1)
            ...
            x_0, x_1 = self._next_batch()
            t = torch.randint(
                low=0, high=self.noiser.num_timesteps,
                size=(x_0.shape[0],),
    ...
    def train(self):
        steps_per_epoch = int(len(self.data_loader) *
                              self.args.repeat_per_epoch)
        self.cache_size = min(
            self.cache_size,
        ...
        for epoch in range(self.args.epochs):
            ...
            for i in range(steps_per_epoch):
                ...
                ema_loss = loss.item() if ema_loss is None \
                    else (ema_loss * ema_loss_w(i) + loss.item() * (1 - ema_loss_w(i)))
                ...
                if self.rank == 0 and ((i + 1) % self.args.log_interval == 0 or i == steps_per_epoch - 1):
                    if self.args.global_rank == 0:
                        self.logger.log_step(i, epoch * steps_per_epoch + i, epoch, ema_loss,
                                             raw_loss, t, dsb=self.dsb, direction=self.direction if self.dsb else '')

                    desc = f'epoch {epoch}, direction {self.direction}, iteration {i}, loss {ema_loss:.14f}' \
                        if self.dsb else f'epoch {epoch}, iteration {i}, loss {ema_loss:.14f}'
        ...
        with torch.no_grad():
            ...
            if self.args.global_rank == 0:
                ...
                if self.dsb:
                    ...
                    if epoch == 0:
                        ps = self.noiser.trajectory_dsb(
                            x_prior, x_data, sample=True)[0]
                    else:
                        ps = self.forward_model.inference(
                            x_data, sample=True)[0]
                else:
                    qs = self.model.inference(x_prior, return_all=True)[1]
                    ps = self.noiser.trajectory(x_prior, x_data)

                x_prior, x_data, qs, ps = x_prior.to(self.device), x_data.to(
                    self.device), qs.to(self.device), ps.to(self.device)

                os.makedirs(os.path.join(self.save_path,
                            'trajectory'), exist_ok=True)
                torch.save({
                    'x_prior': x_prior,
                    'x_data': x_data,
                    'qs': qs,
                ...
                if self.dsb:
                    self.evaluators['Inference'].draw(
                        epoch, iters, qs[-1], subfix=f'_q')
                    self.evaluators['Inference'].draw(
                        epoch, iters, ps[-1], subfix=f'_p')
                else:
                    self.evaluators['Inference'].draw(epoch, iters, qs[-1])

                if last:
                    self.evaluators['Trajectory'].draw(
                        epoch, iters, xs=qs, subfix='_q')
                    self.evaluators['Trajectory'].draw(
                        epoch, iters, xs=ps, subfix='_p')
    ...
    def save_ckpt(self, epoch, iters):
        ...
        torch.save(ckpt, os.path.join(self.save_path,
                   'ckpt', f'ep{epoch}_it{iters}.pth'))
    ...
    def load_ckpt(self):
        ...
        if self.args.ckpt is not None:
            ...
            if self.dsb:
                self.backward_model.load_state_dict(
                    match_ckpt(ckpt['backward_model']), strict=False)
                self.forward_model.load_state_dict(
                    match_ckpt(ckpt['forward_model']), strict=False)
                if "backward_optimizer" in ckpt:
                    self.backward_optimizer.load_state_dict(
                        ckpt['backward_optimizer'])
                    self.forward_optimizer.load_state_dict(
                        ckpt['forward_optimizer'])
            else:
                self.model.load_state_dict(
                    match_ckpt(ckpt['model']), strict=False)
